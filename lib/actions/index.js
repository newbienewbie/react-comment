    ...
    return dispatch=>{
        return Api.fetchReplyListUnder(scope,topicId,replyUnder,page,size)
            .then(info=>{
                ...
                dispatch(fetchReplyListOfCommentReceived(replyUnder,comments,count));
                dispatch(replyListPageChanged(replyUnder,page));
            });
    };
}
 ...
 * @param {Number} count 
 */
export function fetchReplyListOfCommentReceived(commentId,rows,count){
    return {
        type:TYPES.fetchReplyListOfCommentReceived,
        commentId,
        rows,
        count,
    };
...
export function createReply(scope,topicId,replyTo,content){
    return dispatch=>{
        return Api.create(scope,topicId,replyTo,content)
            .then(reply=>{
                dispatch(createReplySucceeded(comment));
                const page=1;
                const size=10; // todo: 待修改
                dispatch(fetchReplyListOfComment(scope,topicId,comment.id,page,size))
            });
    };
}


export function createReplySucceeded(){
    return {
